...
class FileCore():
    def __init__(self, DiskSize:int=256) -> None:
    ...
    def contiguousAllocation(self, file_to_allocated: UserFile, Disk: list):
        """
        磁盘文件连续分配
    ...
    def creatFileOrFolder(self, is_folder: bool, name: str, parent_folder: Folder, file_table: list,
        ...
        if is_folder:
            ...
            if not name == 'root':
                ...
                self.tree.create_node(name,name, parent=str(parent_folder)) # 创建子节点
            return new_folder
        else:
            for node in parent_folder.child_nodes:
            ...
            new_file.disk_position = self.contiguousAllocation(new_file, Disk)
            if new_file.disk_position == 'x':
                print('磁盘空间分配错误')
                return -2
            parent_folder.child_nodes.append(new_file)
            self.tree.create_node(name,name, parent=str(parent_folder))

            return new_file


    def getPath(self, is_folder: bool, target_folder: Folder = None, target_file: UserFile = None):
        """
        利用递归获取文件/文件夹的路径
        ...
        path = self.getPath(True, target_folder=parent_node) + '/' + path_now
        return path
    ...
    def pathToObj(self, path: str, IR: dict, file_table: list, Disk: list, root: Folder):
        """
        通过路径找到文件/文件夹
        ...
        for i in range(1, len(path_node_list)):
            if i == len(path_node_list) - 1:  # 最后一层
                ...
                if IR is None:
                    ...
                    return self.creatFileOrFolder(False, path_node_list[i], parent_node, data=IR['content'], Disk=Disk,
                                            file_table=file_table)
                elif IR["operator"] == "createFolder":
                    return self.creatFileOrFolder(True, path_node_list[i], parent_node, data=None, Disk=Disk,
                                            file_table=file_table)
                else:
                    # 不存在问题
                    ...
                    # 写文件
                        ...
                        # 写数据
                            ...
                            self.clearFileInDisk(target, Disk)
                            target.data = IR["content"]
                            target.size = len(IR["content"])
                            target.disk_position = self.contiguousAllocation(target, Disk)
                            return 1
                        ...
                        if isinstance(target, Folder):
                            ...
                            self.tree.remove_node(str(target))
                            return 1
                        ...
                        if IR["newName"] in child_node_names:
                            ...
                            self.tree.update_node(str(target), tag=IR["newName"])
    ...
    def clearFileInDisk(self, target_file: UserFile, Disk: list):
        """
        在物理磁盘中删除文件信息
        :param Disk: 文件系统磁盘
    ...
    def findObjByName(self, name: str, parent_node):
        """
        ...
        if name in child_node_names:
            ...
            for child_node in parent_node.child_nodes:
                if isinstance(child_node, Folder):
                    result = self.findObjByName(name, child_node)
    ...
    def initFileSystem(self, DiskSize: int = 256):
        ...
        if not os.path.exists('tree.pkl'):
            root_node = self.creatFileOrFolder(True, 'root', None, data=None, Disk=disk, file_table=f_table)

            default_folder_1 = self.creatFileOrFolder(True, 'folder1', root_node, data=None, Disk=disk,
                                                file_table=f_table)
            default_folder_2 = self.creatFileOrFolder(True, 'folder2', root_node, data=None, Disk=disk,
                                                file_table=f_table)
            default_folder_3 = self.creatFileOrFolder(True, 'folder3', root_node, data=None, Disk=disk,
                                                file_table=f_table)

            self.creatFileOrFolder(False, 'test', default_folder_1, data='test_content', Disk=disk,
                            file_table=f_table)
            root_node.child_nodes = [default_folder_1, default_folder_2, default_folder_3]


        return root_node, disk, f_table


    def FileTree(self, parent_node):
        # 是目录
        if isinstance(parent_node, Folder):
            data = []
            child_nodes = list(parent_node.child_nodes)
            for child in child_nodes:
                data.append(self.FileTree(child))
